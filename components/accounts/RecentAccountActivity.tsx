...
export default function RecentAccountActivity() {
  ...
  return (
    <div className="flex flex-col flex-1 p-5 rounded-2xl min-h-0 border border-white/5" style={{ ...glassCard, boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.05)' }}>
      <div className="flex justify-between items-center mb-5 flex-shrink-0 relative">
        <h3 className="text-white text-[15px] font-semibold">Recent Account Activity</h3>
        <div className="flex items-center gap-1.5 p-1 rounded-xl shadow-inner border border-white/5" style={{ background: 'rgba(0,0,0,0.2)' }}>
          <button 
      ...
      <div className="flex flex-col flex-1 overflow-y-auto pr-2" style={{ scrollbarWidth: 'none' }}>
        {currentTransactions.map((tx) => (
          <div key={tx.id} className="flex items-center py-2.5 border-b border-white/5 last:border-0">
            <span className="text-[#5A5870] text-xs w-12 flex-shrink-0">{tx.date}</span>
            <span className="text-white text-sm flex-1 truncate pr-4">{tx.description}</span>
            <span className="text-[10px] uppercase font-medium px-2 py-0.5 rounded-full bg-white/5 text-[#8B899A] mr-4">{tx.category}</span>
            <span className={`text-sm font-medium text-right ${tx.type === 'income' ? 'text-[#4FC9A4]' : 'text-white'}`}>
